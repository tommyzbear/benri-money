...
export class StakeKitClient {
  ...
  async submitTransactionHash(txsID: string, hash: string) {
    await this.request('POST', `/v1/transactions/${txsID}/submit_hash`, {
      hash: hash
    })
  }
  async checkTransactionStatus(txsID: string): Promise<Response> {
    return this.request('GET', `/v1/transactions/${txsID}/status`)
  }

  //given a integrationId, return the details of the integration
  async getIntegrationDetails(integrationId: string): Promise<Response> {
    return this.request('GET', `/v2/yields/${integrationId}`)
  }
