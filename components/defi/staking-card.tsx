...
export function StakingCard({ wallet, chains, onTransactionSuccess }: StakingCardProps) {
    ...
    const stakeKitClient = React.useMemo(() => {
        console.log(
            "Creating StakeKit client with API key:",
            "33bea379-2557-4368-9d3d-09e0b47d6a68"
        );
    ...
    const updateStepStatus = (
        stepId: number,
        status: StakingStep["status"],
        txHash?: string,
        errorMessage?: string
    ) => {
        setStakingSteps((prev) =>
            prev.map((step) =>
                step.id === stepId ? { ...step, status, txHash, errorMessage } : step
            )
        );
    ...
    const handleProgressDialogClose = (open: boolean) => {
        if (open === false && isTransactionInProgress) {
            ...
            if (
                confirm(
                    "Are you sure you want to close? This will not stop any pending transactions in your wallet."
                )
            ) {
    ...
    const handleStake = async () => {
        ...
        try {
            ...
            updateStepStatus(
                ...
                "loading",
                undefined,
                `Processing transaction 1/${activeTransactions.length}`
            );
            ...
            for (let i = 0; i < activeTransactions.length; i++) {
                ...
                updateStepStatus(
                    ...
                    "loading",
                    undefined,
                    `Processing transaction ${i + 1}/${activeTransactions.length
                ...
                try {
                    ...
                    if (jsonTx.gas) {
                        ...
                        console.log(
                            `Adjusted gas limit with buffer: ${currentGas} → ${bufferedGas}`
                        );
                    } else {
                        ...
                        try {
                            ...
                            if (gasEstimate) {
                                ...
                                jsonTx.gas = "0x" + bufferedGas.toString(16);
                                console.log(
                                    `Estimated gas limit with buffer: ${estimatedGas} → ${bufferedGas}`
                                );
                            }
                        } catch (estimateError) {
                    ...
                    console.warn(
                        "Error preparing transaction, continuing with original:",
                        prepError
                    );
                }
                ...
                try {
                    ...
                    throw new Error(
                        `Transaction ${i + 1} failed: ${txError.message || "Unknown error"}`
                    );
                }
            ...
            updateStepStatus(
                2,
                "complete",
                txHashes.length > 0 ? txHashes[txHashes.length - 1].hash : undefined
            );
            ...
            for (const txInfo of txHashes) {
                try {
                    ...
                    console.warn(
                        `Error checking transaction status for ${txInfo.txId}:`,
                        statusError
                    );
                    // Non-fatal, continue
                }
            ...
            if (error instanceof Error && error.message === "Operation cancelled by user") {
                ...
                const failedStepId =
                    stakingSteps.find((step) => step.status === "loading")?.id ||
                    currentStepIndex + 1;
                updateStepStatus(
                    failedStepId,
                    "error",
                    ...
                    <div className="py-4">
                        ...
                        <div className="space-y-4">
                            {stakingSteps.map((step, index) => (
                                <div
                                    key={step.id}
                                    className={`flex items-start p-3 rounded-lg border ${currentStepIndex === index
                        ...
                        <div className="mt-6 flex justify-end">
                            <Button
                                ...
                                disabled={
                                    ...
                                    !stakingSteps.some((step) => step.status === "error")
                                }
                            >
                                {stakingSteps[stakingSteps.length - 1].status === "complete"
                                    ? "Done"
                                    : "Close"}
                            </Button>
                        </div>
